...
def create_csv_file(data):
    ...
    Args:
        data (list): List of dictionaries containing data to write to a CSV file.
    """
...
def main():
    ...
    with open('oscar_winners.csv') as csv_file:
        ...
        # Sanity check to make sure the correct header row is present
        ...
        # Else assume the file is valid and continue
            ...
            for row in reader:
                ...
                # Check received movie title matches the one in the CSV file
                if movie_data['Title'] == row['Movie']:
                    print(f"Processing: {row['IMDB']} {movie_data['Title']}")
                    movies.append(movie_data)
                else:
                    # Replace the movie title with the one from the CSV file
                    # This currently only applies to Birdman but could
                    # potentially apply to other movies in the future.
                    print(f"Processing: {row['IMDB']}, {row['Movie']}")
                    print(f"TITLE MISMATCH: Using OMDb title \"{row['Movie']}\" instead of IMBd \"{movie_data['Title']}\"")
                    mismatched_movie_data = movie_data
                    mismatched_movie_data['Title'] = row['Movie']
                    movies.append(mismatched_movie_data)
